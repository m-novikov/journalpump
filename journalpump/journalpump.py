...
class JournalObjectHandler:
    ...
    def process(self) -> Tuple[bool, int]:
        new_entry = {}
...
class JournalPump(ServiceDaemon, Tagged):
    ...
    def read_single_message(self, reader) -> Tuple[bool, int]:
        try:
    ...
    def read_messages(self, reader, hits, chunk_size) -> Tuple[bool, int]:
        lines = 0
    ...
    def run(self):
        ...
        while self.running:
            ...
            self.ping_watchdog()
            poll_timeout = max(0, self.poll_interval - (time.monotonic() - iteration_start_time))
