...
def test_log_rotator(tmp_path):
    ...
    assert "user-1000@71369830a37346919939502d9aa4d6db-000000000000005c-0005d35f32d73065.journal" in log_files
    assert "user-1000.journal" in log_files
...
def test_journalpump_rotated_files(journalpump_factory, journal_log_dir):
    ...
    lf.rotate()
    messages = stub_sender.get_messages(11, timeout=5)
    assert set(m["MESSAGE"] for m in messages) == {f"Message {i}" for i in range(0, 11)}

    lf.rotate()
    lf.rotate()

    messages = stub_sender.get_messages(20, timeout=5)
    assert set(m["MESSAGE"] for m in messages) == {f"Message {i}" for i in range(11, 31)}
...
def test_journalpump_rotated_files_threshold(journalpump_factory, journal_log_dir, msg_buffer_max_length):
    ...
    for it in range(30 // msg_buffer_max_length):
        messages = stub_sender.get_messages(msg_buffer_max_length, timeout=5)
...
def test_journalpump_rotated_files_threshold_bytes(journalpump_factory, journal_log_dir, size, num_messages):
    ...
    for it in range(30 // num_messages):
        messages = stub_sender.get_messages(num_messages, timeout=5)
...
def test_journalpump_rotated_files_deletion(journalpump_factory, journal_log_dir):
    ...
    def _check_message(message):
        messages = stub_sender.get_messages(1, timeout=5)
    ...
    # Dropped messages 3-10 as file was deleted
    _check_message("Message 11")
